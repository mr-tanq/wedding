...
const { Clock, MapPin, Brain, Camera, Heart, Music, Search, Check, X, Trophy, Upload, Send, ThumbsUp, Sparkles, Wine, Flame, Disc3, Radio, Star, Gift, Quote } = Lucide;
...
const MUSIC_BLOCKS = [
  { id: "greek", name: "Greek Traditional", time: "22:30", color: "#C9A24B", songs: [
    { t: "Zorba — Theodorakis", v: 24 }, { t: "Ximeroni — Glykeria", v: 18 },
    { t: "Fraoules Stomati — Marinella", v: 12 }, { t: "Ta Deilina — Dalaras", v: 9 },
  ]},
  { id: "retro", name: "80s / 90s / Trance", time: "00:00", color: "#D4AF37", songs: [
    { t: "Sandstorm — Darude", v: 42 }, { t: "Children — Robert Miles", v: 38 },
    { t: "Blue — Eiffel 65", v: 29 }, { t: "Around the World — ATC", v: 21 },
    { t: "What Is Love — Haddaway", v: 19 },
  ]},
  { id: "party", name: "Party Classics", time: "01:00", color: "#E6C15A", songs: [
    { t: "Mr. Brightside — The Killers", v: 51 }, { t: "Don't Stop Me Now — Queen", v: 48 },
    { t: "September — Earth Wind & Fire", v: 33 }, { t: "Everybody — Backstreet Boys", v: 27 },
  ]},
  { id: "metal", name: "Metal Block", time: "02:00", color: "#B8862E", songs: [
    { t: "Master of Puppets — Metallica", v: 37 }, { t: "Painkiller — Judas Priest", v: 22 },
    { t: "Chop Suey! — System of a Down", v: 29 }, { t: "Raining Blood — Slayer", v: 14 },
    { t: "Rosanna — Rotting Christ", v: 11 },
  ]},
  { id: "late", name: "Late-Night Chaos", time: "03:00", color: "#A67729", songs: [
    { t: "Strobe — Deadmau5", v: 26 }, { t: "Teardrop — Massive Attack", v: 18 },
...
const css = `
  ...
  :root {
    --bg: #08070a; --bg-soft: #0f0d12; --ink: #f5ecd7; --ink-dim: #a89978;
    --gold: #d4af37; --gold-soft: #c9a24b; --gold-deep: #8a6a1e;
    --line: rgba(212, 175, 55, 0.15);
    --glass: rgba(255, 250, 235, 0.035); --glass-strong: rgba(255, 250, 235, 0.06);
  }
  * { box-sizing: border-box; }
  ...
  .gold { color: var(--gold); }
  .app-bg { min-height: 100vh; background: radial-gradient(1200px 600px at 50% -10%, rgba(212,175,55,0.12), transparent 60%), radial-gradient(800px 400px at 80% 110%, rgba(212,175,55,0.06), transparent 60%), var(--bg); position: relative; overflow-x: hidden; }
  .app-bg::before { content: ""; position: fixed; inset: 0; pointer-events: none; background-image: url("data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='180' height='180'><filter id='n'><feTurbulence type='fractalNoise' baseFrequency='0.9' numOctaves='2' stitchTiles='stitch'/><feColorMatrix values='0 0 0 0 0.85  0 0 0 0 0.75  0 0 0 0 0.45  0 0 0 0.08 0'/></filter><rect width='100%' height='100%' filter='url(%23n)'/></svg>"); opacity: 0.35; mix-blend-mode: overlay; z-index: 0; }
  .glass { background: var(--glass); backdrop-filter: blur(14px) saturate(130%); -webkit-backdrop-filter: blur(14px) saturate(130%); border: 1px solid var(--line); border-radius: 18px; }
  .glass-strong { background: var(--glass-strong); backdrop-filter: blur(18px) saturate(140%); -webkit-backdrop-filter: blur(18px) saturate(140%); border: 1px solid rgba(212, 175, 55, 0.22); border-radius: 20px; }
  .hairline { height: 1px; background: linear-gradient(90deg, transparent, rgba(212,175,55,0.45), transparent); }
  .btn-gold { background: linear-gradient(180deg, #e6c15a 0%, #c9a24b 60%, #a67729 100%); color: #0a0908; font-weight: 600; letter-spacing: 0.04em; text-transform: uppercase; font-size: 12px; padding: 14px 20px; border-radius: 999px; border: none; cursor: pointer; box-shadow: 0 8px 24px -8px rgba(212,175,55,0.5), inset 0 1px 0 rgba(255,255,255,0.4); transition: transform 0.2s ease; }
  .btn-gold:active { transform: translateY(1px); }
  .btn-ghost { background: transparent; color: var(--gold); border: 1px solid rgba(212,175,55,0.35); padding: 12px 18px; border-radius: 999px; font-size: 12px; letter-spacing: 0.08em; text-transform: uppercase; cursor: pointer; }
  .tab-bar { position: fixed; bottom: 0; left: 0; right: 0; background: rgba(8, 7, 10, 0.85); backdrop-filter: blur(20px) saturate(140%); -webkit-backdrop-filter: blur(20px) saturate(140%); border-top: 1px solid var(--line); z-index: 50; padding: 8px 6px calc(8px + env(safe-area-inset-bottom)); }
  .tab-row { display: grid; grid-template-columns: repeat(6, 1fr); max-width: 520px; margin: 0 auto; }
  .tab { display: flex; flex-direction: column; align-items: center; gap: 3px; padding: 8px 4px; color: var(--ink-dim); cursor: pointer; font-size: 9px; letter-spacing: 0.05em; text-transform: uppercase; transition: color 0.2s; background: none; border: none; }
  ...
  .shimmer { background: linear-gradient(90deg, transparent, rgba(212,175,55,0.18), transparent); background-size: 200% 100%; animation: shim 2.4s linear infinite; }
  ...
  @keyframes pulse { 0% { box-shadow: 0 0 0 0 rgba(212,175,55,0.6); } 70% { box-shadow: 0 0 0 10px rgba(212,175,55,0); } 100% { box-shadow: 0 0 0 0 rgba(212,175,55,0); } }
  input, textarea, select { background: rgba(255,250,235,0.04); border: 1px solid var(--line); color: var(--ink); padding: 14px 16px; border-radius: 12px; font-family: 'Inter', sans-serif; font-size: 14px; width: 100%; outline: none; }
  input:focus, textarea:focus { border-color: var(--gold-soft); }
  input::placeholder, textarea::placeholder { color: var(--ink-dim); }
  .chip { display: inline-flex; align-items: center; gap: 6px; padding: 6px 12px; border-radius: 999px; background: rgba(212,175,55,0.08); border: 1px solid rgba(212,175,55,0.2); color: var(--gold); font-size: 10px; letter-spacing: 0.1em; text-transform: uppercase; }
  ...
  @media (max-width: 380px) { .h-display { font-size: 36px; } .tab { font-size: 8px; } }
`;
  ...
  return (
    <header style={{ position: "relative", zIndex: 1, paddingTop: 32, paddingBottom: 8 }}>
      <div style={{ maxWidth: 520, margin: "0 auto", padding: "0 20px", textAlign: "center" }}>
        ...
        <h1 className="h-display">Kostas <span className="amp">&</span> Chara</h1>
      </div>
    </header>
  );
}
...
function TimelineScreen() {
  ...
  const events = useMemo(() => TIMELINE.map((e) => ({ ...e, date: timeStrToDate(e.time, WEDDING_DATE) })), []);
  const { currentIdx, nextIdx, progress } = useMemo(() => {
    let cur = -1;
    for (let i = 0; i < events.length; i++) if (now >= events[i].date) cur = i;
    const next = cur + 1 < events.length ? cur + 1 : -1;
    let prog = 0;
    if (cur >= 0 && next >= 0) {
      const span = events[next].date - events[cur].date;
      prog = Math.min(1, Math.max(0, (now - events[cur].date) / span));
  ...
  return (
    <div className="reveal">
      <div className="chip" style={{ marginBottom: 14 }}><span className="pulse-dot" /> Live</div>
      <div className="glass-strong" style={{ padding: 24, marginBottom: 20 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 18 }}>
          <div>
            ...
            <div className="serif" style={{ fontSize: 28, lineHeight: 1.1 }}>{nextEvent ? nextEvent.label : "Encore"}</div>
          </div>
        ...
        <div className="progress-rail"><div className="progress-fill shimmer" style={{ width: `${overallProgress * 100}%`, transition: "width 1s linear" }} /></div>
        ...
        {events.map((e, i) => {
          ...
          return (
            <div key={e.time} style={{ position: "relative", display: "flex", gap: 16, paddingBottom: 22 }}>
              {i < events.length - 1 && <div style={{ position: "absolute", left: 19, top: 40, bottom: 0, width: 1, background: isPast ? "rgba(212,175,55,0.5)" : "rgba(212,175,55,0.12)" }} />}
              <div style={{ width: 40, height: 40, borderRadius: "50%", flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center", background: isCurrent ? "linear-gradient(180deg, #e6c15a, #a67729)" : isPast ? "rgba(212,175,55,0.15)" : "rgba(255,250,235,0.04)", border: isCurrent ? "none" : `1px solid ${isPast ? "rgba(212,175,55,0.4)" : "var(--line)"}`, color: isCurrent ? "#0a0908" : isPast ? "var(--gold)" : "var(--ink-dim)", boxShadow: isCurrent ? "0 0 24px rgba(212,175,55,0.4)" : "none", position: "relative", zIndex: 1 }}>
              ...
              <div style={{ flex: 1, paddingTop: 2 }}>
                ...
                <div className="serif" style={{ fontSize: 21, color: isPast ? "var(--ink-dim)" : "var(--ink)", marginBottom: 2, textDecoration: isPast ? "line-through" : "none", textDecorationColor: "rgba(212,175,55,0.3)" }}>{e.label}</div>
                <div style={{ fontSize: 12, color: "var(--ink-dim)" }}>{e.note}</div>
              </div>
            </div>
          );
        })}
...
function TableScreen() {
  ...
  const matches = useMemo(() => {
    ...
    const results = [];
    TABLES.forEach((t) => t.people.forEach((p) => { if (p.toLowerCase().includes(q)) results.push({ table: t, person: p }); }));
  ...
  return (
    <div className="reveal">
      ...
      <div style={{ position: "relative", marginBottom: 16 }}>
        ...
        <input value={query} onChange={(e) => { setQuery(e.target.value); setSelected(null); }} placeholder="Type your name…" style={{ paddingLeft: 44 }} />
      </div>
      {!selected && matches.length > 0 && (
        <div className="glass" style={{ padding: 6, marginBottom: 16 }}>
          {matches.map((m, i) => (
            <button key={i} onClick={() => { setSelected(m); setQuery(m.person); }} style={{ width: "100%", textAlign: "left", padding: "12px 14px", background: "transparent", border: "none", color: "var(--ink)", display: "flex", justifyContent: "space-between", alignItems: "center", cursor: "pointer", borderRadius: 10, fontSize: 14 }}>
              <span>{m.person}</span>
      ...
      {!selected && !query && (
        <>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginTop: 16 }}>
            {TABLES.map((t) => (
              <button key={t.n} onClick={() => setSelected({ table: t, person: null })} className="glass" style={{ padding: 14, textAlign: "left", cursor: "pointer", color: "var(--ink)" }}>
...
function TableCard({ match }) {
  ...
  return (
    <div className="glass-strong reveal" style={{ padding: 24, marginBottom: 16 }}>
      {person && (<div style={{ marginBottom: 16 }}><div className="eyebrow" style={{ marginBottom: 4 }}>Hello</div><div className="serif" style={{ fontSize: 24 }}>{person}</div></div>)}
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 }}>
        <div><div className="eyebrow" style={{ marginBottom: 4 }}>Your Table</div><div className="serif" style={{ fontSize: 30, lineHeight: 1 }}>{table.name}</div></div>
        <div className="serif gold" style={{ fontSize: 48, lineHeight: 0.9 }}>0{table.n}</div>
      </div>
      <div className="hairline" style={{ margin: "14px 0" }} />
      <div style={{ fontSize: 13, color: "var(--ink-dim)", fontStyle: "italic", marginBottom: 16 }}>"{table.desc}"</div>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginBottom: 18 }}>
        {table.people.map((p) => {
          ...
          return (
            <div key={p} style={{ display: "flex", alignItems: "center", gap: 8, padding: "6px 12px 6px 6px", background: isYou ? "rgba(212,175,55,0.12)" : "rgba(255,250,235,0.03)", border: `1px solid ${isYou ? "rgba(212,175,55,0.4)" : "var(--line)"}`, borderRadius: 999 }}>
              <div style={{ width: 26, height: 26, borderRadius: "50%", background: isYou ? "linear-gradient(135deg, #e6c15a, #8a6a1e)" : "rgba(212,175,55,0.15)", color: isYou ? "#0a0908" : "var(--gold)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 10, fontWeight: 600 }}>{initials}</div>
...
function VenueMap({ tables, highlight, onSelect }) {
  ...
  return (
    <div style={{ position: "relative", width: "100%", aspectRatio: "1 / 1.1", background: "radial-gradient(ellipse at top, rgba(212,175,55,0.08), transparent 60%), rgba(255,250,235,0.02)", border: "1px solid var(--line)", borderRadius: 16, overflow: "hidden" }}>
      <div style={{ position: "absolute", left: "35%", right: "35%", top: 12, height: 18, border: "1px solid rgba(212,175,55,0.4)", borderRadius: 4, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 8, letterSpacing: "0.3em", color: "var(--gold)", textTransform: "uppercase" }}>Stage</div>
      <div style={{ position: "absolute", left: "30%", right: "30%", bottom: 12, height: 40, border: "1px dashed rgba(212,175,55,0.3)", borderRadius: 8, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 8, letterSpacing: "0.3em", color: "var(--ink-dim)", textTransform: "uppercase" }}>Dancefloor</div>
      {tables.map((t) => {
        const p = positions[t.n] || { x: 50, y: 50 };
        const isHi = t.n === highlight;
        return (
          <button key={t.n} onClick={() => onSelect && onSelect(t)} style={{ position: "absolute", left: `${p.x}%`, top: `${p.y}%`, transform: "translate(-50%, -50%)", width: 26, height: 26, borderRadius: "50%", background: isHi ? "linear-gradient(135deg, #e6c15a, #8a6a1e)" : "rgba(8,7,10,0.8)", border: `1px solid ${isHi ? "var(--gold)" : "rgba(212,175,55,0.3)"}`, color: isHi ? "#0a0908" : "var(--gold)", fontSize: 10, fontWeight: 600, cursor: onSelect ? "pointer" : "default", boxShadow: isHi ? "0 0 20px rgba(212,175,55,0.6)" : "none" }}>{t.n}</button>
        );
      })}
    </div>
  );
...
function QuizScreen() {
  ...
  const leaderboard = [
    { name: "Sem", score: 92 }, { name: "Despina", score: 88 },
    { name: "Andreas", score: 84 }, { name: "Marina", score: 79 },
    { name: "You", score: answers.reduce((s, a) => s + a.points, 0), isYou: true },
  ].sort((a, b) => b.score - a.score);
  useEffect(() => { setStartTime(Date.now()); }, [idx]);

  if (idx >= QUIZ.length) return <QuizDone answers={answers} leaderboard={leaderboard} onRestart={() => { setIdx(0); setAnswers([]); setSelected(null); setLocked(false); }} />;
  ...
  const pick = (opt) => {
    ...
    const points = (correct ? 100 : 0) + (correct ? Math.max(0, Math.floor(50 - elapsed * 5)) : 0);
    setSelected(opt); setLocked(true);
    setTimeout(() => {
      setAnswers((a) => [...a, { q: q.q, correct, points }]);
      setSelected(null); setLocked(false); setIdx((i) => i + 1);
    }, 1100);
  };
  ...
  return (
    <div className="reveal">
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 18 }}>
        ...
        <div className="chip" style={{ padding: "4px 10px" }}><Trophy size={10} /> {answers.reduce((s, a) => s + a.points, 0)} pts</div>
      </div>
      <div className="progress-rail" style={{ marginBottom: 24 }}><div className="progress-fill" style={{ width: `${(idx / QUIZ.length) * 100}%`, transition: "width 0.4s" }} /></div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        {q.opts.map((opt, i) => {
          ...
          return (
            <button key={i} disabled={locked} onClick={() => pick(i)} className="glass" style={{ padding: "16px 18px", textAlign: "left", cursor: locked ? "default" : "pointer", color: "var(--ink)", display: "flex", justifyContent: "space-between", alignItems: "center", borderColor: isCorrect ? "rgba(148, 200, 120, 0.6)" : isWrong ? "rgba(220, 100, 100, 0.5)" : "var(--line)", background: isCorrect ? "rgba(148, 200, 120, 0.1)" : isWrong ? "rgba(220, 100, 100, 0.08)" : "var(--glass)", transition: "all 0.3s" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
        {leaderboard.slice(0, 5).map((p, i) => (
          <div key={p.name} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 14px", borderRadius: 10, background: p.isYou ? "rgba(212,175,55,0.08)" : "transparent", border: p.isYou ? "1px solid rgba(212,175,55,0.25)" : "1px solid transparent" }}>
...
function PhotosScreen() {
  ...
  const upload = (e) => {
    ...
    files.forEach((f) => {
      ...
      setPhotos((list) => [{ id: Date.now() + Math.random(), url, cat: "Funny", author: "You", likes: 0, liked: false }, ...list]);
    });
  };
  const toggleLike = (id) => setPhotos((list) => list.map((p) => p.id === id ? { ...p, liked: !p.liked, likes: p.likes + (p.liked ? -1 : 1) } : p));
  ...
  return (
    <div className="reveal">
      ...
      <div style={{ color: "var(--ink-dim)", fontSize: 13, marginBottom: 20, fontStyle: "italic" }}>Every hidden moment the official camera missed.</div>
      <div style={{ display: "flex", gap: 10, marginBottom: 18 }}>
        <button className="btn-gold" style={{ flex: 1 }} onClick={() => fileRef.current?.click()}><Upload size={14} style={{ verticalAlign: "-2px", marginRight: 8 }} />Upload</button>
        <button className="btn-ghost" onClick={() => fileRef.current?.click()}><Camera size={14} style={{ verticalAlign: "-2px" }} /></button>
        <input ref={fileRef} type="file" accept="image/*" multiple onChange={upload} style={{ display: "none" }} />
      </div>
      <div style={{ display: "flex", gap: 8, overflowX: "auto", paddingBottom: 12, marginBottom: 12, marginLeft: -20, marginRight: -20, paddingLeft: 20, paddingRight: 20 }}>
        {CATS.map((c) => (
          <button key={c} onClick={() => setCat(c)} style={{ flexShrink: 0, padding: "8px 14px", borderRadius: 999, background: cat === c ? "var(--gold)" : "transparent", color: cat === c ? "#0a0908" : "var(--ink-dim)", border: `1px solid ${cat === c ? "var(--gold)" : "var(--line)"}`, fontSize: 11, letterSpacing: "0.1em", textTransform: "uppercase", cursor: "pointer", fontWeight: cat === c ? 600 : 400 }}>{c}</button>
        ))}
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
        {filtered.map((p) => (
          <div key={p.id} style={{ position: "relative", borderRadius: 14, overflow: "hidden", aspectRatio: "3 / 4", background: p.url.startsWith("blob") ? "#222" : p.url }}>
            ...
            <div style={{ position: "absolute", top: 8, left: 8 }}><span className="chip" style={{ padding: "3px 8px", fontSize: 9, background: "rgba(0,0,0,0.4)" }}>{p.cat}</span></div>
            <div style={{ position: "absolute", bottom: 8, left: 10, right: 10, display: "flex", justifyContent: "space-between", alignItems: "flex-end" }}>
              ...
              <button onClick={() => toggleLike(p.id)} style={{ background: "rgba(0,0,0,0.4)", backdropFilter: "blur(8px)", border: "1px solid rgba(255,255,255,0.15)", borderRadius: 999, padding: "4px 10px", color: p.liked ? "var(--gold)" : "white", fontSize: 11, display: "flex", alignItems: "center", gap: 5, cursor: "pointer" }}>
...
function seedPhotos() {
  ...
  return Array.from({ length: 8 }, (_, i) => ({
    id: i + 1, url: palette[i], cat: cats[i % cats.length],
    author: authors[i % authors.length], likes: Math.floor(Math.random() * 40) + 3, liked: false,
  }));
}
    ...
    <div className="reveal">
      ...
      <div className="glass-strong" style={{ padding: 18, marginBottom: 24 }}>
        ...
        <div style={{ display: "flex", gap: 6, marginBottom: 10, flexWrap: "wrap" }}>
          {TYPES.map((t) => (
            <button key={t} onClick={() => setType(t)} style={{ padding: "6px 12px", borderRadius: 999, fontSize: 10, letterSpacing: "0.1em", textTransform: "uppercase", background: type === t ? "var(--gold)" : "transparent", color: type === t ? "#0a0908" : "var(--ink-dim)", border: `1px solid ${type === t ? "var(--gold)" : "var(--line)"}`, cursor: "pointer", fontWeight: type === t ? 600 : 400 }}>{t}</button>
          ))}
        </div>
        ...
        <button className="btn-gold" onClick={submit} style={{ width: "100%" }}><Send size={12} style={{ verticalAlign: "-1px", marginRight: 8 }} />Send to Kostas & Chara</button>
      </div>
      <div className="eyebrow" style={{ marginBottom: 14 }}>The Wall · {messages.length}</div>
      <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        {messages.map((m) => (
          <article key={m.id} className="glass" style={{ padding: 20 }}>
            ...
            <div className="serif" style={{ fontSize: 17, lineHeight: 1.5, fontStyle: "italic" }}>{m.text}</div>
          </article>
        ))}
      </div>
    </div>
...
function MusicScreen() {
  ...
  const vote = (title) => {
    ...
    setBlocks((bs) => bs.map((b) => b.id !== activeBlock ? b : { ...b, songs: b.songs.map((s) => s.t === title ? { ...s, v: s.v + 1 } : s) }));
  };
  const suggest = () => {
    if (!suggestion.trim()) return;
    setBlocks((bs) => bs.map((b) => b.id !== activeBlock ? b : { ...b, songs: [...b.songs, { t: suggestion.trim(), v: 1, suggested: true }] }));
    setSuggestion("");
  };
  return (
    <div className="reveal">
      ...
      <div style={{ color: "var(--ink-dim)", fontSize: 13, marginBottom: 20, fontStyle: "italic" }}>Top 3 in each block get played. Cast wisely.</div>
      <div style={{ display: "flex", gap: 8, overflowX: "auto", paddingBottom: 12, marginBottom: 18, marginLeft: -20, marginRight: -20, paddingLeft: 20, paddingRight: 20 }}>
        {blocks.map((b) => {
          const active = b.id === activeBlock;
          return (
            <button key={b.id} onClick={() => setActiveBlock(b.id)} style={{ flexShrink: 0, padding: "10px 16px", borderRadius: 12, background: active ? "rgba(212,175,55,0.1)" : "transparent", border: `1px solid ${active ? b.color : "var(--line)"}`, cursor: "pointer", textAlign: "left", minWidth: 140 }}>
          ...
          {sorted.map((s, i) => {
            ...
            return (
              <div key={s.t} style={{ position: "relative", padding: "12px 14px", borderRadius: 10, overflow: "hidden", background: "rgba(255,250,235,0.03)", border: `1px solid ${isTop3 ? "rgba(212,175,55,0.25)" : "var(--line)"}` }}>
                <div style={{ position: "absolute", left: 0, top: 0, bottom: 0, width: `${pct}%`, background: `linear-gradient(90deg, ${block.color}22, ${block.color}08)`, transition: "width 0.5s" }} />
                <div style={{ position: "relative", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  ...
                  <button onClick={() => vote(s.t)} disabled={hasVoted} style={{ background: hasVoted ? "rgba(212,175,55,0.15)" : "transparent", border: `1px solid ${hasVoted ? block.color : "var(--line)"}`, color: hasVoted ? block.color : "var(--ink)", padding: "5px 10px", borderRadius: 999, cursor: hasVoted ? "default" : "pointer", fontSize: 11, display: "flex", alignItems: "center", gap: 5 }}>
      ...
      <div className="glass" style={{ padding: 16 }}>
        ...
        <div style={{ fontSize: 10, color: "var(--ink-dim)", marginTop: 10, fontStyle: "italic" }}>DJ approves additions. Songs must match the block's mood.</div>
      </div>
    </div>
  );
}